...
structure mapT =SplayMapFn (struct type ord_key = int
                                val compare = Int.compare end)
fun first (a,_) =a
fun second (_,b)=b
...
val mA=mapT.empty
(*fun myfil f [] a =a
  | myfil f (a::ls) (c,b) = if f a =true then (a::ca)*)
fun hash a=
  ( (SetT.foldl (op+) 0 (SetT.map ((fn x=>IntInf.toInt (IntInf.pow (2, x))))
   (SetT.filter (fn x=> x<23) a)) ),(SetT.foldl (op+) 0 (SetT.map ((fn
   x=>IntInf.toInt (IntInf.pow (2, (x mod 22)))))
      (SetT.filter (fn x=> x>22) a)) ));
fun inMap k= 
let val i = mapT.find(mA,first k)
in
  if (i = NONE) then false else SetT.member(Option.valOf i,second k)
end
fun addMap k =mapT.insert(mA,first k,second k);
fun dfs2 q q1 = if
  (Queue.isEmpty q) then dfs q1 (Queue.mkQueue())  else dfs q q1 
and dfs q1 q2 = 
let val i = Queue.dequeue q1
in
  if(List.length (first i) =0) then second i else (if (inMap (hash (second i))) =true then
    dfs2 q1 q2 
  else(
  addMap (hash (second i) );
