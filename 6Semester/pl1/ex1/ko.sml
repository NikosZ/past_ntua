  ...
  duplicates i []
      ...
      in
        (if n = 0 then List.take (sqrs,k) else 
          let val i = conv (List.take (sqrs,(k))) (dik n) []
    in
      duplicates i []
      end)
          end
...
fun solver [] ls = ls
  | solver (a::b) ls = let val i = solver(b) ls
          in
            sumk  (map (add (a*a,1)) i) (funky (List.length b) a)
            []
end
fun is_happy1 (a,b) =
...
fun take_in n = sum_a (solver (rev (digitalize n)) []);
...
val _ =print  (String.toInt (happy ( (List.hd  args))))
