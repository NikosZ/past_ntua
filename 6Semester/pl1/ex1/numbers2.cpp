...
uint* den_kanw_diktia(uint n)
{
	...
	return no_fft(den_kanw_diktia(ceil((1.0*n)/2)),den_kanw_diktia(n/2));
}
uint* funky(uint len,uint back)
{
	uint *temp= new uint[mvl]();
	temp=temp+730;//hackia
	...
	return temp=no_fft(temp,den_kanw_diktia(len));
}
uint* solver(list<int> digits)
{
	...
	temp=solver(digits);
	temp-=sqrs[back];
	return sum(funky(size,back),temp);
}
...
int main(int argc,char *argv[] )
{
	...
	//reverse(a.begin(),a.end());
	    printf("%u\n",sum_a(solver(b),memoization)-sum_a(solver(a),memoization)+is_happy(sum_digits(A),memoization));
	        return 0;
	        }	
