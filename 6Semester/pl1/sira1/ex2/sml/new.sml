...
fun hash a=
 SetT.foldl (op+) 0 (SetT.map ((fn x=>IntInf.toInt (IntInf.pow (2, x))))  a);
(*fun enq q (a,b) map= 
let val i = hash b
in
  if IntBinaryMap.find(map, i) =NONE then
    Queue.enqueue(q,(a,b));IntBinaryMap.insert(map,i,)*)
fun dfs2 q q1 map= if
