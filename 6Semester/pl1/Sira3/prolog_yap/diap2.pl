...
getSol(G,G,_,[]).
getSol(S,G,L,[H|T]):- hash_f(G,O),!,get_assoc(O,L,par(H,LL)),!,getSol(S,LL,L,T).
...
%getSol(Goal,LL,[]):-hash_f(Goal,F),get_assoc(F,LL,K),hash_f(K,F2),write(F2),nl,write(F),nl,F2==F.
%getSol(Goal,LL,[H|T]):-hash_f(Goal,F),get_assoc(F,LL,H),getSol(H,LL,T).
