...
search(G,G,_Q,_T,L,LS):-getSol(G,L,LS).
search(S,G,Q,T,L,LS):-next_moves(S,Q,T,L,L2),nb_queue_dequeue(Q,NS),search(NS,G,Q,T,L2,LS).


next_moves(S,Q,T,L,L3):-moves(S,X),trie_f(S,X,T,Xn,L,L3),!,list_to_Q(Xn,Q).
...
diapragmateysi(S,L):-format_to_chars(S,'',Cur1),isfinal(Goal1),list_to_num(Cur1,Cur),list_to_num(Goal1,Goal),nb_queue(Q),trie_open(T),hash_f(Cur,Hash),list_to_assoc([Hash-par(0,Cur)],L1),!,search(Cur,Goal,Q,T,L1,L),!. %
%,write("A"),getSol(Cur,LL,L).
%,reverse(LL,Sol2),atomic_list_concat(Sol2,L),!.


getSol(Goal,LL,[]):-hash_f(Goal,F),get_assoc(F,LL,par(0,G2)).
getSol(Goal,LL,[H|T]):-hash_f(Goal,F),get_assoc(F,LL,par(H,G2)),!,write(H),nl,getSol(G2,LL,T).
