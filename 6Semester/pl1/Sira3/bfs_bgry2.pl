...
addParrents(_,[],[]).
addParrents(S,[[X,H]|T],[X|T2]):- hash_f(X,Hash),\+parrents(Hash,_,_),assert(parrents(Hash,S,H)),addParrents(S,T,T2).
...
diapragmateysi(S,L):- string_to_list(S,Cur22),isfinal(Goal1),list_to_num(Goal1,Goal),list_to_num(Cur22,Cur),assert(queue([])),hash_f(Cur,CurH),assert(parrents(CurH,CurH,0)),search(Cur,Goal),hash_f(Goal,GK),getSolution(GK,PP),reverse(PP,AS),atomic_list_concat(AS,AA),string_codes(L,AA),!.
