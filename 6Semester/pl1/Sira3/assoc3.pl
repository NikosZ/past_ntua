...
%
%queue([]).
search(Goal,Goal,A,A,_).
search(State,Goal,List,L3,Q):- next_moves(State,M_C,List,L2),!,list_to_Q(M_C,Q,[H|T]),!,search(H,Goal,L2,L3,T).
...
diapragmateysi(S,L):- string_to_list(S,Cur22),isfinal(Goal1),list_to_num(Goal1,Goal),list_to_num(Cur22,Cur),assert(queue([])),hash_f(Cur,CurH),list_to_assoc([CurH-parrents(CurH,0)],L1),search(Cur,Goal,L1,L4,[]),!,hash_f(Goal,GK),getSolution(GK,CurH,PP,L4),reverse(PP,AS),atomic_list_concat(AS,AA),string_codes(L,AA),!.
